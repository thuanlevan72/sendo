...
const ProductDetail = () => {
  ...
  const postData = async (cartItem) => {
    try {
      const response = await axios.post(
        `https://api-sendo.vercel.app/giohang`,
        cartItem
      );
  ...
  const putData = async (id, cartItem) => {
    try {
      const response = await axios.put(
        `https://api-sendo.vercel.app/giohang/${id}`,
        cartItem
      );
      return response.data;
    } catch (err) {
  ...
  function handleCart() {
    if (currentUser == null) {
      ...
      getData()
        .then((data) => {
          //console.log(data)
          const existingObjectIndex = data.findIndex(
            (obj) =>
              obj.userId == cartItem.userId &&
              obj.idProduct == cartItem.idProduct
          );
  ...
  const getProductId = async () => {
    try {
      const response = await axios.get(
        `https://api-sendo.vercel.app/sanpham?id=${id}`
      );
      //console.log(response.data)
      return response.data;
    } catch (error) {
  ...
  const handleIncrease = () => {
    if (slide != length) {
      setSlide(slide + 1);
      setImage(newDetail.images[slide]);
      showBorder(slide);
    } else {
      setSlide(length);
      setImage(newDetail.images[length - 1]);
      showBorder(length - 1);
    }
  };
  ...
  const handleDecrease = () => {
    if (slide != 1) {
      setSlide(slide - 1);
      console.log(slide);
      setImage(newDetail.images[slide - 2]);
      showBorder(slide - 2);
    } else {
      setSlide(1);
      setImage(newDetail.images[0]);
      showBorder(0);
    }
  };
  ...
  return (
    <>
      {contextHolder}
      {showHeader && (
        <div className="sticky bg-white top-16 px-3 md:px-5 lg:px-10 z-50 text-[14px]">
        ...
        <div className="product lg:flex rounded-xl bg-white">
          <div className="content-left w-full p-5">
            <div className="relative w-full">
              <div className=" image w-full h-full flex items-center justify-between">
                <span className="relative">
                  <span
                    className="bg-gray-100 hover:bg-gray-300  absolute left-2 p-2 rounded"
                    onClick={() => handleDecrease()}>
                    <AiOutlineLeft className="ml-0" />
                  </span>
                </span>
                <img src={image} className="w-full h-full" />
                <span className="relative">
                  <span
                    className="bg-gray-100 hover:bg-gray-300 absolute right-2 p-2 rounded"
                    onClick={() => handleIncrease()}>
                    <AiOutlineRight className="ml-0" />
                  </span>
                </span>
              </div>
              <div className="infor-img w-full flex justify-between absolute bottom-2">
                <div className="number bg-zinc-200 px-3 rounded-3xl text-sm flex items-center">
                  {slide}/{length}
                </div>
                <div className="flex">
                  <div className="bg-zinc-200 p-2 rounded text-sm">
                    {" "}
                    <AiOutlineShareAlt className="text-base" />
                  </div>
                  <div className="bg-zinc-200 p-2 ml-2 rounded text-sm">
                    {" "}
            ...
            <div className="suggestion width-full bg-white flex">
              {newDetail &&
                newDetail.images &&
                newDetail.images.map((item, index) => {
                  return (
                    <div
                      className="bg-white w-20 cursor-pointer m-1 rounded border-2 hover:border-red-500"
                      onClick={() => {
                        handleNav(index);
                      }}
                      id={index}>
          ...
          <div className="content-right p-5 w-full">
            <div className="title">
              <h1 className="flex items-center text-xl">
                <img
                  src="https://media3.scdn.vn/img4/2022/08_17/nbMbRRfJY3a65DmxuyUH.png"
                  alt="Shop's badge"
                  className="h-4 mr-2"
                />
                {newDetail.title}
            ...
            <div className="brand text-sm mt-2">
              ...
              <a className="text-blue-600" href="/thuong-hieu/elmich">
                {newDetail.brand}
              </a>
            </div>
            <div className="price mt-2">
              ...
              <span>
                <NumericFormat
                  type="text"
                  value={
                    (newDetail.price / (100 - newDetail.discountPercentage)) *
                    100 *
                    10000
                  }
                  thousandsGroupStyle="thousand"
            ...
            <div className="amount text-sm mt-5 flex items-center ">
              ...
              <div className="flex items-center">
                <button
                  className="px-2 bg-gray-200 rounded h-8 text-xl text-gray-400 mx-0.5 border hover:bg-gray-100"
                  onClick={handleMinus}>
                  <BsDashLg />
                </button>
            ...
            <div className="flex mt-5">
              <button
                className="w-1/2 bg-gray-200 rounded h-[48px] text-base font-bold text-black mx-0.5 border"
                onClick={handleCart}>
          ...
          <div className="detail-left w-full lg:w-[50%]">
            <div className="sticky top-32 bg-white  p-6 rounded-xl">
              ...
              <div className="flex">
                <img
                  className="w-20 "
                  src="https://media3.scdn.vn/img4/2022/09_27/nqrquod949Ya6lzfHNQX.png"
                />
                <div>
                  ...
                  <div className="flex items-center">
                    ...
                    <span className="text-gray-500 point pl-2 relative">
                      {newDetail.rating}
                    </span>
              ...
              <div className="flex text-center">
                ...
                <div className="flex-1">
                  <h1 className="font-bold text-base m-0">
                    {newDetail.tyLePhanHoi}%
                  </h1>
                  <h2 className="text-m font-[400] leading-5">
                    Tỷ lệ phản hồi
                  </h2>
                </div>
                <div className="flex-1">
                  <h1 className="font-bold text-base m-0">
                    {newDetail.shopPhanHoi}%
                  </h1>
                  <h2 className="text-m font-[400] leading-5">Shop phản hồi</h2>
                </div>
              </div>
              <div className="flex">
                <div className="flex w-[45%] bg-gray-200 rounded mr-3 justify-center items-center py-2 font-bold text-sm">
                  {" "}
                  <RiHeartAddLine className="text-lg" />{" "}
                  <p className="pl-2 m-0"> Theo dõi shop</p>
                </div>
                <div className="flex w-[45%] bg-gray-200 rounded mr-3 justify-center items-center py-2 font-bold text-sm">
                  {" "}
                  <IoStorefrontOutline className="text-lg" />{" "}
                  <p className="pl-2 m-0"> Vào shop</p>{" "}
                </div>
                <div className="w-[10%] flex bg-gray-200 rounded mr-3 justify-center items-center py-2 font-bold text-sm">
              ...
              <div className="bg-gradient-to-b from-white to-red-300 rounded-lg flex items-center">
                <button className="custom-prev absolute bg-gray-200 p-[10px] rounded z-50 text-[18px] left-[-10px] ">
                  <BsChevronLeft />
                </button>
                <Swiper
                  ...
                  {newData.map((slide) => (
                    <SwiperSlide
                      key={slide.id}
                      className="bg-white shadow-lg shadow-black-500 hover:shadow-gray-500 h-[200px] rounded-lg">
                ...
                <button className="custom-next absolute right-[-10px] bg-gray-200 p-[10px] rounded z-50 text-[18px]">
                  <BsChevronRight />
                </button>
              </div>
            </div>
          ...
          <div className="detail-right">
            <div className=" bg-white p-6 rounded-xl " id="general">
              ...
              <div>
                <button
                  id="btn"
                  className="bg-gray-200 w-full rounded font-bold text-[14px] py-1 text-gray-600"
                  onClick={handleClick}>
                  Xem thêm
                </button>
              </div>
            ...
            <div className=" bg-white p-6 rounded-xl mt-6 " id="review">
              <img
                className="mx-auto"
                src="https://web-static.scdn.vn/sendo-communication-rating/863edd0-web/media/rating-empty.f56ae9e22805ed6a864d1a540bea0947.svg"></img>
              <h1 className="text-sm font-bold text-center">
                Sản phẩm chưa có đánh giá
              </h1>
              <h1 className="text-sm font-bold text-center text-gray-500">
            ...
            <div className=" bg-white p-6 rounded-xl mt-6 " id="question">
              ...
              <div>
                <button
                  id="btn"
                  className="bg-gray-200 w-full rounded font-bold text-[14px] py-1 text-gray-600 flex justify-center items-center"
                  onClick={handleClick}>
                  {" "}
                  <BsChatSquareText className="text-[20px] mr-2" />
                  Hỏi shop ngay
                </button>
              </div>
            </div>
        ...
        <div className="favorite mt-6" id="foryou">
          ...
          <div className="mt-5 flex w-full flex-wrap justify-between">
            {newData.map((product) => (
              <div
                key={product.id}
                className="w-[45%] lg:w-[23%] xl:w-[15%] m-2 bg-white shadow-lg shadow-black-500 hover:shadow-gray-500 rounded-lg">
                <Linkroute to={`/productdetail/${product.id}`}>
                  <div className="bg-white h-[300px] rounded-lg relative">
                    <img
                      className="w-[100%] h-36 object rounded-t-[10px]"
